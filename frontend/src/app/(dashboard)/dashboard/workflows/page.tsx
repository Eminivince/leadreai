      ...
      {!isLoading && workflows.length === 0 && (
        <div className="border border-dashed border-[color:var(--rule)] bg-[color:var(--paper-3)]/50 rounded-sm p-10 text-center">
          ...
          <Link
            ...
            className="mt-5 inline-flex items-center gap-2 border border-[color:var(--rule)] hover:border-[color:var(--ink)] px-4 py-2 rounded-full  text-[13px] text-[color:var(--ink)] transition"
          >
            Go to Tables →
          </Link>
        </div>
...
function WorkflowCard({ workflow }: { workflow: Workflow }) {
  ...
  return (
    ...
    >
      ...
      <h3 className=" text-[22px] leading-[1.1] text-[color:var(--ink)] mb-1">
        {workflow.name}
      </h3>
      {workflow.description && (
        <p className=" text-[13px] leading-[1.45] text-[color:var(--ink-2)] mb-3 line-clamp-2">
          {workflow.description}
        </p>
      )}
      <div className="mt-3 flex items-center justify-between gap-2 font-mono text-[10px] tracking-[0.18em] uppercase text-[color:var(--ink-3)]">
        <span>
          {colCount} col{colCount === 1 ? '' : 's'}
          {enrichedCount > 0 && (
            <span className="text-[color:var(--forest)] normal-case tracking-normal  ml-1 text-[11px]">
