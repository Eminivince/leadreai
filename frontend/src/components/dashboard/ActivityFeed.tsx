        ...
        <div className="flex items-center gap-1 liquid-glass rounded-full p-0.5">
          {TABS.map(t => (
            <button key={t} onClick={() => setFilter(t)}
              className={`relative z-[1] px-2 py-0.5 text-[10.5px] font-body rounded-full transition ${filter === t ? 'bg-white text-black' : 'text-white/70 hover:text-white'}`}>
              {t}
            </button>
          ))}
        </div>
        ...
        {FEED.map((f, i) => (
          <div key={i} className="flex items-start gap-3 py-2 border-b border-white/5 last:border-b-0">
            <div className="relative mt-1 shrink-0">
              <span className={`block w-1.5 h-1.5 rounded-full ${DOT_COLORS[f.tone] ?? 'bg-white/50'}`} />
            </div>
