    ...
    <GlassCard className="p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {display.map((j, i) => {
          ...
          const ringColor = j.status === 'failed' ? 'bg-rose-400' : j.status === 'complete' || j.status === 'done' ? 'bg-emerald-400' : 'bg-white';
          const isRunning = j.status !== 'complete' && j.status !== 'done' && j.status !== 'failed' && j.status !== 'cancelled';
          return (
            <div key={i} className="rounded-xl bg-white/[0.03] border border-white/10 p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 pt-1">
                {j.status === 'failed' && onRetry && 'jobId' in j && (
                  <button onClick={() => onRetry(String((j as Record<string, unknown>)['jobId']))}
                    className="text-[11px] font-body font-medium text-white bg-white/10 hover:bg-white/15 rounded-full px-2.5 py-1 inline-flex items-center gap-1">
