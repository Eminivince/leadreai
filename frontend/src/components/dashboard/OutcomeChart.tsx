    ...
    <GlassCard className="p-5">
        ...
        <div className="flex items-center gap-1 liquid-glass rounded-full p-1 shrink-0">
          {['7d', '14d', '30d', 'QTD'].map((r) => (
            <button key={r} onClick={() => setRange(r)}
              className={`relative z-[1] px-2.5 py-1 text-[11px] font-body rounded-full transition ${range === r ? 'bg-white text-black' : 'text-white/70 hover:text-white'}`}>
              {r}
            </button>
          ))}
        </div>
      ...
      <div className="flex items-center gap-4 md:gap-6 mt-2 mb-2 flex-wrap">
        <div className="flex items-baseline gap-1.5">
          <span className="w-2 h-2 rounded-full bg-white translate-y-[-2px]" />
